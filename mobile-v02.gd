...
const APP_FRIEND := 7
const APP_REWARDS := 8
const ARENA_ENTRY_COSTS: Array[int] = [0, 100, 500]
const ARENA_WIN_PRIZES: Array[int] = [100, 250, 1200]
...
func start_arena_search() -> void:
	var entry: int = ARENA_ENTRY_COSTS[clampi(selected_arena, 0, ARENA_ENTRY_COSTS.size() - 1)]
...
func handle_multiplayer_message(payload: Dictionary) -> void:
	match str(payload.get("type", "")):
		...
		"match_started":
			...
			if match_source == "arena":
				var entry: int = ARENA_ENTRY_COSTS[clampi(int(payload.get("arena", selected_arena)), 0, ARENA_ENTRY_COSTS.size() - 1)]
