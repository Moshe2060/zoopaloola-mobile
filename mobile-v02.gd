...
func _ready() -> void:
	...
	floating_portals_home_texture = load("res://assets/ui/world/floating-portals-home-v2.webp") as Texture2D
...
func home_mode_rect(index: int, viewport_size: Vector2) -> Rect2:
	var unit := minf(viewport_size.x / 1280.0, viewport_size.y / 720.0)
	if index == 0:
		return Rect2(Vector2(944.0, 423.0) * unit, Vector2(281.0, 63.0) * unit)
	if index == 1:
		return Rect2(Vector2(64.0, 423.0) * unit, Vector2(272.0, 63.0) * unit)
	if index == 2:
		return Rect2(Vector2(455.0, 475.0) * unit, Vector2(372.0, 80.0) * unit)
	return Rect2()
...
func home_top_control_rects(viewport_size: Vector2) -> Dictionary:
	var unit := minf(viewport_size.x / 1280.0, viewport_size.y / 720.0)
	return {
		"coin": Rect2(Vector2(842.0, 18.0) * unit, Vector2(143.0, 47.0) * unit),
		"gems": Rect2(Vector2(991.0, 18.0) * unit, Vector2(143.0, 47.0) * unit),
		"sound": Rect2(Vector2(-100.0, -100.0) * unit, Vector2(1.0, 1.0) * unit),
		"help": Rect2(Vector2(1147.0, 18.0) * unit, Vector2(55.0, 55.0) * unit),
		"settings": Rect2(Vector2(1207.0, 18.0) * unit, Vector2(55.0, 55.0) * unit),
	}
...
func home_nav_rect(index: int, viewport_size: Vector2) -> Rect2:
	var unit := minf(viewport_size.x / 1280.0, viewport_size.y / 720.0)
	return Rect2(Vector2(651.0 + float(index) * 168.0, 621.0) * unit, Vector2(164.0, 82.0) * unit)

func home_character_rect(viewport_size: Vector2) -> Rect2:
	var unit := minf(viewport_size.x / 1280.0, viewport_size.y / 720.0)
	return Rect2(Vector2(315.0, 621.0) * unit, Vector2(164.0, 82.0) * unit)

func home_core_rect(viewport_size: Vector2) -> Rect2:
	var unit := minf(viewport_size.x / 1280.0, viewport_size.y / 720.0)
	return Rect2(Vector2(483.0, 621.0) * unit, Vector2(164.0, 82.0) * unit)
...
func handle_frontend_touch(screen_pos: Vector2) -> void:
	...
	if app_screen == APP_HOME:
		...
		if home_character_rect(viewport_size).has_point(screen_pos):
			...
			return
		if home_core_rect(viewport_size).has_point(screen_pos):
			app_screen = APP_SHOP
			shop_page = SHOP_PAGE_EFFECTS
			return
...
func draw_portal_world_home(viewport_size: Vector2) -> void:
	...
	# Subtle live energy is layered over the production art.
	for spark in 14:
		var phase := menu_elapsed * (0.32 + float(spark % 4) * 0.07) + float(spark) * 1.71
		var spark_pos := Vector2(fmod(phase * 91.0, viewport_size.x), 110.0 * unit + fmod(float(spark * 71), 360.0) * unit)
		draw_circle(spark_pos, (1.2 + float(spark % 3)) * unit, Color(0.78, 0.95, 1.0, 0.20 + sin(phase * 2.0) * 0.08))
	var portal_pulse := (sin(menu_elapsed * 2.6) + 1.0) * 0.5
	draw_arc(Vector2(640.0, 323.0) * unit, (176.0 + portal_pulse * 4.0) * unit, 0.15, PI - 0.15, 48, Color(0.42, 0.93, 1.0, 0.20 + portal_pulse * 0.12), 3.0 * unit, true)

	# Dynamic content is drawn only inside the sculpted empty frames.
	draw_string(ui_font, Vector2(421.0, 88.0) * unit, "שערי הקרב" if ui_language == "he" else "BATTLE GATES", HORIZONTAL_ALIGNMENT_CENTER, 430.0 * unit, int(39.0 * unit), Color("dcfaff"))
	var profile := home_profile_rect(viewport_size)
	draw_string(ui_font, profile.position + Vector2(62.0, 27.0) * unit, profile_name, HORIZONTAL_ALIGNMENT_LEFT, profile.size.x - 72.0 * unit, int(18.0 * unit), Color.WHITE)
	draw_string(ui_font, profile.position + Vector2(62.0, 48.0) * unit, player_level_label(), HORIZONTAL_ALIGNMENT_LEFT, profile.size.x - 72.0 * unit, int(11.0 * unit), Color("79e6ff"))
	draw_string(ui_font, profile.position + Vector2(2.0, 41.0) * unit, profile_initial(), HORIZONTAL_ALIGNMENT_CENTER, 48.0 * unit, int(24.0 * unit), Color.WHITE)

	var coin := home_coin_rect(viewport_size)
	var gems := home_gems_rect(viewport_size)
	draw_string(ui_font, coin.position + Vector2(42.0, 34.0) * unit, str(player_coins), HORIZONTAL_ALIGNMENT_CENTER, coin.size.x - 49.0 * unit, int(19.0 * unit), Color.WHITE)
	draw_string(ui_font, gems.position + Vector2(42.0, 34.0) * unit, "0", HORIZONTAL_ALIGNMENT_CENTER, gems.size.x - 49.0 * unit, int(19.0 * unit), Color.WHITE)
	for control_data in [[home_settings_rect(viewport_size), "⚙"], [home_help_rect(viewport_size), "✉"]]:
	...
	for mode in 3:
		...
		draw_string(ui_font, mode_rect.position + Vector2(0.0, mode_rect.size.y * 0.64), portal_titles[mode], HORIZONTAL_ALIGNMENT_CENTER, mode_rect.size.x, int((25.0 if mode == 2 else 20.0) * unit), Color.WHITE)
	var dock_labels := ["דמויות", "ליבות", ui_text("shop"), ui_text("rewards")] if ui_language == "he" else ["HEROES", "CORES", "SHOP", "REWARDS"]
	var dock_centers := [397.0, 565.0, 733.0, 901.0]
	for i in 4:
		var center := Vector2(dock_centers[i], 655.0) * unit
		if i == 0:
			draw_circle(center + Vector2(0.0, -8.0) * unit, 14.0 * unit, Color("77ddff"))
			draw_arc(center + Vector2(0.0, 13.0) * unit, 18.0 * unit, PI, TAU, 16, Color("77ddff"), 6.0 * unit, true)
		elif i == 1:
			draw_circle(center, 18.0 * unit, Color("4be5ff", 0.22))
			draw_circle(center, 10.0 * unit, Color("72edff"), false, 4.0 * unit, true)
			draw_circle(center, 4.0 * unit, Color.WHITE)
		else:
			draw_home_nav_icon(i - 2, center, unit)
		draw_string(ui_font, Vector2((dock_centers[i] - 70.0) * unit, 696.0 * unit), dock_labels[i], HORIZONTAL_ALIGNMENT_CENTER, 140.0 * unit, int(14.0 * unit), Color.WHITE)
	draw_pending_invite_banner(viewport_size)
